...
def register_user(request):
    ...
    if User.objects.filter(username=req_body['username']).exists():
        ...
        return HttpResponse(json.dumps({"error": "Username already exists. Please choose a different username."}),
    ...
    return HttpResponse(json.dumps(data), content_type='application/json', status=status.HTTP_201_CREATED)